...
<div class="picker">
  ...
  <div class="row" id="nodes">{nodes_static}</div>
</div>
...
(function () {{
  ...
  function showGame(game) {{
    groups.forEach(function (g) {{
      g.style.display = g.getAttribute("data-game") === game ? "flex" : "none";
    }});
  }}
  ...
  nbtns.forEach(function (b) {{
    b.addEventListener("click", function () {{
      nbtns.forEach(function (x) {{ x.setAttribute("aria-pressed", x === b ? "true" : "false"); }});
      current.src = b.getAttribute("data-src");
      show(current.src);
    }});
  }});
  showGame(current.game);
...
def main() -> None:
    ...
    def node_btns(nodes: list[dict]) -> str:
        out = []
        for i, v in enumerate(nodes):
            pressed = ' aria-pressed="true"' if i == 0 else ""
            out.append(
                f'<button class="nbtn" data-src="{esc(v["src"])}"{pressed}>'
                f'<span class="t">{esc(v["n"])}</span><span class="s">{esc(v["s"])}</span></button>'
            )
        return "".join(out)

    nodes_static = "".join(
        f'<div class="ngroup" data-game="{k}"{" style=display:flex" if idx == 0 else ""}>'
        f"{node_btns(v)}</div>"
        for idx, (k, v) in enumerate(views.items())
    )
    page = SHELL.format(
        title=esc(teams),
        league=esc(league),
