...
def main() -> None:
    ...
    # 显式系列比分陈述（"A 2:0 B"式，双方队名齐全）在末段的数量
    # 教训 2026-08-26：单队名 "KT 2:0"（预测/单边喊）会把 G1 结束误判成整场结束
    teams_low = [t for t in teams if t]

    def _series_claim(text: str) -> bool:
        low = (text or "").lower()
        if len(teams_low) != 2:
            return False
        a, b = teams_low
        return any(
            (a in low and s in low and b in low)
            for s in ("2:0", "2-0", "2:1", "2-1")
            for a, b in ((a, b), (b, a))
        )

    score_claims = [r for r in last2 if _series_claim(r.get("text") or "")]
